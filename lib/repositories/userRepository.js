    ...
    return {
        ...
        getWeb: function(req, res, next) {
            ...
            var sql = 'SELECT keyword.id as keywordId, keyword.keyword, user.id as userId, user.firstName, user.lastName, user.functionDescription, user.profileImage ' +
                ...
                'WHERE keyword.id IN (SELECT Keyword_id FROM user_has_keyword WHERE User_id = ' + req.params.id + ') AND user.id != ' + req.params.id;
            ...
            pool.query(sql, function(err, result, fields) {
                if (err) { console.log(err); return next(err); }
                    ...
                    pool.query(userSql, function(err, result, fields) {
                        if (err) { console.log(err); return next(err); }
                        else {
                            user = {
                                "id": result[0].id,
