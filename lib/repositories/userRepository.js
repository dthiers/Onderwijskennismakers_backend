...
module.exports = function(pool) {
    ...
    return {
        ...
        getById: function(req, res, next){
            var sql = 'SELECT id, firstName, lastName, email, description, functionDescription, profileImage, isAdmin, isFrozen, facebook, twitter, skype, linkedIn FROM user WHERE id = ' + req.params.id;
        ...
        getDetails: function(req, res, next) {
            ...
            pool.query(sql, [req.params.id], function
                (err, result, fields){
                if (err) { console.log(err); return next(err); }
                else {
                    res.return(result);
            ...
            if(id && typeof id === 'number'){
                ...
                pool.query(sql, [req.params.id, req.params.id], function(err, result, fields) {
                    if (err) { console.log(err); return next(err); }
                        ...
                        var contentSql =    'SELECT content.*, concat(user.firstName, " ", user.lastName) AS "author", user.profileImage, AVG(content_rating.value) as rating ' +
                                            'FROM content ' +
        ...
        getContent: function(req, res, next){
            var sql =   'SELECT content.*, user.profileImage ' +
                        'FROM content ' +
