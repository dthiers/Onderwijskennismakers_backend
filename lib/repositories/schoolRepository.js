...
module.exports = function(pool) {
    ...
    return {
        ...
        updateExpert: function(req, res, next){
            var sql =   'UPDATE school_has_user SET ? WHERE School_id = ? AND User_id = ?';
            
            pool.query(sql, [req.body, req.params.id, req.params.user_id], function (err, result, fields){
                if (err) {
                    console.log(err);
                    return next(err);
                }
                else {
                    res.json("Successfully updated an entity in the school_has_user table.");
                }
            })  
        },
        
        deleteExpert: function(req, res, next){
            var sql =   'DELETE FROM school_has_user WHERE School_id = ? AND User_id = ?';
            
            pool.query(sql, [req.params.id, req.params.user_id], function(err, result, fields){
                if (err){
                    console.log(err);
                    return next(err);
                }
                else {
                    res.json("Successfully removed an entity in the school_has_user table.");
                }
            })  
        },
